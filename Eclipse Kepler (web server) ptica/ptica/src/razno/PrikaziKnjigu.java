...
public class PrikaziKnjigu extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int i = 0;
            Cookie[] kolacici = request.getCookies(); // niz koji sadrÅ¾i kolaÄ�iÄ‡e
            int duz_kolacici = kolacici.length;
            boolean nadjen = false; // da li je pronaÄ‘en kolaÄ�iÄ‡ indeks_knjige  
            
            // prolazim kroz kolaÄ�iÄ‡e dok kolaÄ�iÄ‡ sa imenom indeks_knjige nije pronaÄ‘en 
            while (i < duz_kolacici && !nadjen) {
                ...
                if (je_knjiga_indeks) {
                   ...
                   hSesija2.setAttribute("indeks_knjige", kolacic_vr);
                   nadjen = true;
                }
                i++;
            }
            
            // String vrednost = String.valueOf(hSesija2.getAttribute("indeks_knjige"));
            request.getRequestDispatcher("ptica_knj.jsp").forward(request, response); // ptica_index.jsp
